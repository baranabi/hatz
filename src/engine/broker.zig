...
pub fn call(allocator: std.mem.Allocator, run: *sim.RunState, payload: BrokerCallRequestPayload) !BrokerCallResponsePayload {
    ...
    if (std.mem.eql(u8, payload.method, "ib.beacons")) {
        ...
        const resp = try brokerResponse(allocator, run, payload.analystId, payload.method, 0, tick, false, result);
        allocator.free(result.beacons);
        return resp;
    }
    if (std.mem.eql(u8, payload.method, "ib.all_capabilities")) {
        const caps = try capabilityRange(allocator, 16);
        const result = ResultAllCapabilities{ .capabilities = caps };
        const resp = try brokerResponse(allocator, run, payload.analystId, payload.method, 0, tick, false, result);
        allocator.free(caps);
        return resp;
    }
    if (std.mem.eql(u8, payload.method, "ib.benign_organizations")) {
        // Collect benign organization IDs from the seeded population.
        var list: std.ArrayList(types.OrganizationId) = .empty;
        for (run.organizations) |org| {
            if (org.org_type == .BENIGN) {
                try list.append(allocator, org.id);
            }
        }
        const ids = try list.toOwnedSlice(allocator);
        const result = ResultOrganizations{ .organizationIds = ids };
        const resp = try brokerResponse(allocator, run, payload.analystId, payload.method, 0, tick, false, result);
        allocator.free(ids);
        return resp;
    }
    if (std.mem.eql(u8, payload.method, "ib.terrorist_organizations")) {
        // Collect terrorist organization IDs from the seeded population.
        var list: std.ArrayList(types.OrganizationId) = .empty;
        for (run.organizations) |org| {
    ...
    if (std.mem.eql(u8, payload.method, "ib.known_terrorist_hats")) {
        // Collect hats whose advertised color is TERRORIST (overt terrorists).
        var list: std.ArrayList(types.HatId) = .empty;
        for (run.hats) |hat| {
            if (hat.advertised_color == .TERRORIST) {
                try list.append(allocator, hat.id);
            }
        }
        const ids = try list.toOwnedSlice(allocator);
        const result = ResultHatIds{ .hatIds = ids };
        const resp = try brokerResponse(allocator, run, payload.analystId, payload.method, 0, tick, false, result);
        allocator.free(ids);
        return resp;
    }
    if (std.mem.eql(u8, payload.method, "ib.members")) {
        ...
        const hat_ids: []const types.HatId = if (org_id < run.organizations.len)
            run.organizations[@as(usize, @intCast(org_id))].members
        else
            &[_]types.HatId{};
        const ids = try allocator.dupe(types.HatId, hat_ids);
        const result = ResultMembers{ .hatIds = ids };
        const resp = try brokerResponse(allocator, run, payload.analystId, payload.method, 0, tick, false, result);
        allocator.free(ids);
        return resp;
    }
    if (std.mem.eql(u8, payload.method, "ib.hat_advertised_color")) {
        ...
        const color: types.HatAdvertisedColor = if (hat_id < run.hats.len)
            run.hats[@as(usize, @intCast(hat_id))].advertised_color
        else
            .NOT_A_KNOWN_HAT;
    ...
    if (std.mem.eql(u8, payload.method, "ib.capabilities")) {
        ...
        if (!hatExists(run, @intCast(hat_id))) {
            const caps = try allocator.alloc(types.CapabilityId, 0);
            const resp = try brokerResponse(allocator, run, payload.analystId, payload.method, 0, tick, false,
        ...
        const resp = try brokerResponse(allocator, run, payload.analystId, payload.method, payment, tick, true,
            ResultCapabilities{ .capabilities = caps });
        allocator.free(caps);
        return resp;
    }
    if (std.mem.eql(u8, payload.method, "ib.meeting_times")) {
        ...
        // Unknown hat → no noise, empty result.
        if (!hatExists(run, @intCast(hat_id))) {
            const ticks = try allocator.alloc(types.Tick, 0);
            const resp = try brokerResponse(allocator, run, payload.analystId, payload.method, 0, tick, false,
    ...
    if (std.mem.eql(u8, payload.method, "ib.meeting_participants")) {
        ...
        if (participants) |p| allocator.free(p);
        return resp;
    }
...
test "free queries have charged=0 and noisy=false" {
    ...
    var empty_args = try std.json.ObjectMap.init(allocator, &.{}, &.{});
    defer empty_args.deinit(allocator);
    const resp = try call(allocator, run, .{
